...
async function enableHighAvailabilityConfig(page) {
  try {
    ...
    for (const frame of page.frames()) {
      const switchBtn = await frame.$('button[role="switch"][aria-label="Enable High Availability Configuration"]');
      if (switchBtn) {
        const isChecked = await frame.evaluate(el => el.getAttribute("aria-checked"), switchBtn);

        if (isChecked === "false") {
          await switchBtn.click();
          console.log("✅ High Availability has been enabled (inside iframe).");
        } else {
          console.log("ℹ️ High Availability is already enabled (inside iframe).");
        }
    ...
    if (!found) {
      throw new Error("❌ Could not find the High Availability switch button in any frame.");
    }
  } catch (err) {
    console.error("⚠️ Error enabling High Availability:", err.message);
  }
}
...
async function calculatePricing(sl,row, mode,isFirst, isLast) {
  ...
  let browser, page;
    ...
    try {
      ...
      //await sleep(500);
      if (row["High Availability"].toLowerCase() === "ha") {
