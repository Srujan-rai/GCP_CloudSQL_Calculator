...
async function calculatePricing(sl,row, mode,isFirst, isLast) {
  ...
  let browser, page;
    ...
    try {
      ...
      //await setNumberOfvCPUs(page, Number(row["vCPUs"])); ucc-11786
      //await sleep(1000)
      //await setAmountOfMemory(page,Number(row["RAM"])); ucc-11789
      //await sleep(1000);
      //await enableHighAvailabilityconfig(page,row["High Availability"]);
      //await sleep(500); 
