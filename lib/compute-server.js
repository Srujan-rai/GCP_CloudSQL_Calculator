...
app.post('/cloudsql', async (req, res) => {
  const row = req.body;
...
async function CloudSqlModal(page) {
  console.log('Waiting for modal to find "Cloud SQL"...');
  
  // The XPath selector remains the same.
  const cloudSqlXPath = `//h2[contains(text(), "Cloud SQL")]/ancestor::div[@role="button"]`;

  try {
    // 1. Wait until the browser can find the element.
    // This runs the check inside the browser until it returns a truthy value (the element).
    console.log('Waiting for the element to exist...');
    await page.waitForFunction(
      (xpath) => {
        const element = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
        return element; // Returns the element object if found, or null if not.
      },
      { timeout: 10000 },
      cloudSqlXPath
    );
    ...
    // 2. Now, execute the click within the browser.
    const clicked = await page.evaluate((xpath) => {
      // Find the element again.
      const elementToClick = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
      if (elementToClick) {
        // We must cast it to an HTMLElement to access the .click() method.
    ...
    if (clicked) {
      console.log('✅ Cloud SQL clicked successfully.');
    } else {
      throw new Error('❌ Found the element, but failed to click it.');
    }

  } catch (error) {
    console.error('❌ Could not find or click the "Cloud SQL" tile within the timeout.');
    throw error;
  }
}
...
async function serviceType(page, serviceTypeName) {
  console.log(`Attempting to select Service Type: "${serviceTypeName}"...`);

  try {
    // This selector targets the part of the dropdown that shows the current value.
    const currentValueSelector = 'div[aria-labelledby="ucc-2"] span[jsname="Fb0Bif"]';
    await page.waitForSelector(currentValueSelector, { visible: true, timeout: 5000 });
    const currentValue = await page.$eval(currentValueSelector, el => el.textContent.trim());

    // If it's already selected, we do nothing.
    if (currentValue === serviceTypeName) {
      console.log(`✅ Service Type "${serviceTypeName}" is already selected.`);
      return;
    }

    // 1. Click the main dropdown element to open the list.
    const dropdownOpenerSelector = 'div[aria-labelledby="ucc-2"]';
    await page.click(dropdownOpenerSelector);
    console.log('✅ Dropdown opened.');

    // This XPath finds the clickable list item (`li`) containing the desired text.
    const optionXPath = `//li[@role="option" and .//span[text()="${serviceTypeName}"]]`;

    // 2. Wait for the option to exist in the browser's DOM.
    await page.waitForFunction(
      (xpath) => {
        const element = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
        return !!element; // Return true if element is found
      },
      { timeout: 5000 },
      optionXPath
    );
    console.log(`✅ Option "${serviceTypeName}" is present in the list.`);

    // 3. Now, execute the click within the browser.
    const clicked = await page.evaluate((xpath) => {
      const elementToClick = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
      if (elementToClick) {
        // We cast to HTMLElement to call the .click() method.
        (elementToClick).click();
        return true; // Indicate success
      }
      return false; // Indicate failure
    ...
    if (!clicked) {
      throw new Error('Element was found, but the click action failed inside the browser.');
    }

    console.log(`✅ Successfully selected "${serviceTypeName}".`);

  } catch (error) {
    console.error(`❌ Failed to select Service Type "${serviceTypeName}".`);
    throw error;
  }
}
...
async function selectRegion(page, regionName, index = 0) {
  const labelText = 'Region'; // The label is now fixed to 'Region'.
  console.log(`Attempting to select "${regionName}" from dropdown "${labelText}" (index ${index})...`);

  try {
    // XPath is 1-based, so we add 1 to our 0-based index.
    const dropdownXPath = `(//div[@role='combobox' and .//span[text()='${labelText}']]) [${index + 1}]`;

    // 1. Check if the specified dropdown exists.
    const dropdownExists = await page.evaluate((xpath) => {
      return !!document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
    }, dropdownXPath);

    if (!dropdownExists) {
      console.log(`🟡 Warning: Dropdown with label "${labelText}" at index ${index} was not found. Skipping.`);
      return;
    }
    console.log(`✅ Dropdown "${labelText}" (index ${index}) found.`);

    // 2. Get the current value to see if we need to change it.
    const currentValue = await page.evaluate((xpath) => {
        const dropdownNode = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
        const valueNode = dropdownNode.querySelector('span[jsname="Fb0Bif"]');
        return valueNode ? valueNode.textContent.trim() : null;
    }, dropdownXPath);

    if (currentValue && currentValue.includes(regionName)) {
      console.log(`✅ Option "${currentValue}" matching "${regionName}" is already selected.`);
      return;
    }
    ...
    // 4. Define XPath for the option to be selected.
    const optionXPath = `//li[@role="option" and contains(., "${regionName}")]`;

    // 5. Wait for the option to appear and then click it.
    await page.waitForFunction(
    ...
    if (!clicked) {
      throw new Error('Option was found, but the click action failed.');
    }

    console.log(`✅ Successfully selected "${regionName}".`);

  } catch (error) {
    console.error(`❌ Failed to select "${regionName}" from dropdown "${labelText}".`);
    throw error;
  }
}

async function cloudSqlEdition(page) {
  const editionName = 'Enterprise Plus';
  console.log(`Attempting to select SQL Edition: "${editionName}"...`);

  try {
    const labelXPath = `//label[text()='${editionName}']`;

    // This command runs inside the browser
    const selected = await page.evaluate((xpath) => {
      ...
      if (label) {
        ...
        // Click the label only if its input is not already checked
        if (input && !input.checked) {
           label.click();
        }
        return true; // Found and handled the element
      }
      return false; // Did not find the element
    }, labelXPath);

    if (selected) {
      console.log(`✅ Edition "${editionName}" selected (or was already selected).`);
    } else {
      throw new Error(`Could not find the label for "${editionName}".`);
    }

  } catch (error) {
    console.error(`❌ Failed to select Edition "${editionName}".`);
    throw error;
  }
}

async function specifyUsageTimeToggle(page) {
  const toggleLabel = 'Specify usage time for each instance';
  console.log(`Attempting to toggle switch for: "${toggleLabel}"...`);

  try {
    // This robust XPath finds the label text, navigates to the shared parent container,
    // and then finds the clickable button with role="switch".
    const buttonXPath = `//div[text()='${toggleLabel}']/ancestor::div[contains(@class, 'Fcwn5b')]//button[@role='switch']`;

    // The logic runs inside the browser to find and click the button.
    const result = await page.evaluate((xpath) => {
      const button = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
      
      if (button) {
        // Get the state *before* clicking to report the change.
    ...
    if (result.success) {
      // Determine the new state based on the initial state.
      const finalState = result.initialState === 'true' ? 'OFF' : 'ON';
      console.log(`✅ Switch for "${toggleLabel}" was toggled. Its new state should be ${finalState}.`);
    } else {
      throw new Error(`Could not find the switch associated with "${toggleLabel}". Reason: ${result.reason}`);
    }

  } catch (error) {
    console.error(`❌ Failed to toggle switch for "${toggleLabel}".`);
    throw error;
  }
}
...
async function setNumberOfInstances(page, no_of_instance) {
  ...
  try {
      ...
      await page.type(instancesInputSelector, targetValue, { delay: 500 });
...
async function SqlInstanceType(page, instanceType) {
  // The label is inferred from the `aria-label` in your HTML snippet.
  const labelText = 'SQL instance type';
  console.log(`Attempting to select Instance Type: "${instanceType}"...`);

  try {
    // This XPath finds the dropdown control by its visible label.
    const dropdownXPath = `//div[@role='combobox' and .//span[text()='${labelText}']]`;

    // 1. Check if the dropdown exists on the page.
    const dropdownExists = await page.evaluate((xpath) => {
      return !!document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
    }, dropdownXPath);

    if (!dropdownExists) {
      console.log(`🟡 Warning: Dropdown for "${labelText}" was not found. Skipping.`);
      return;
    }
    console.log(`✅ Dropdown for "${labelText}" found.`);

    // 2. Get the currently selected value to see if we need to do anything.
    const currentValue = await page.evaluate((xpath) => {
        const dropdownNode = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
        const valueNode = dropdownNode.querySelector('span[jsname="Fb0Bif"]'); // Assumes standard structure for the selected value
        return valueNode ? valueNode.textContent.trim() : null;
    }, dropdownXPath);

    // Check if the correct instance is already selected (case-insensitive check).
    if (currentValue && currentValue.toLowerCase() === instanceType.toLowerCase()) {
      console.log(`✅ Instance Type "${instanceType}" is already selected.`);
      return;
    }

    // 3. Click the dropdown to open the list.
    await page.evaluate((xpath) => {
        const elementToClick = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
        if (elementToClick) (elementToClick).click();
    }, dropdownXPath);
    ...
    // 4. Find the option in the list. This XPath is case-insensitive to handle 'N' vs 'n'.
    const instanceTypeLower = instanceType.toLowerCase();
    const optionXPath = `//li[@role="option" and .//span[@jsname='K4r5Ff' and translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')='${instanceTypeLower}']]`;
    
    // 5. Wait for the option to appear, then click it.
    await page.waitForFunction(
      (xpath) => !!document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue,
      { timeout: 5000 },
      optionXPath
    ...
    if (!clicked) {
      throw new Error('Instance type option was found, but the click action failed.');
    }

    console.log(`✅ Successfully selected Instance Type "${instanceType}".`);

  } catch (error) {
    console.error(`❌ Failed to select Instance Type "${instanceType}".`);
    throw error;
  }
}


async function setNumberOfvCPUs(page, count) {
  // This selector targets the number input field based on its unique class 'CNaLNe'.
  // You may need to adjust this selector if the website's structure changes.
  const vCPUInputSelector = 'input.CNaLNe';

  try {
    console.log(`Attempting to set vCPUs to: ${count}`);

    // 1. Wait for the input field to be present and visible on the page.
    await page.waitForSelector(vCPUInputSelector, { visible: true });

    // 2. Click the input field three times to select all the text inside it.
    // This is a reliable way to ensure the old value is fully selected.
    await page.click(vCPUInputSelector, { clickCount: 3 });

    // 3. Type the new number. This will automatically overwrite the selected text.
    // We convert the 'count' to a string as page.type() requires a string.
    await page.type(vCPUInputSelector, String(count));

    console.log(`Successfully set vCPUs to ${count}.`);

  } catch (error) {
    console.error(`Error: Failed to set the number of vCPUs for value ${count}.`);
    console.error(error);
    // Optionally re-throw the error if you want the main script to stop execution
    throw new Error(`Could not set vCPU count: ${error.message}`);
  }
}


async function setAmountOfMemory(page, amount) {
  try {
    console.log(`Attempting to set memory to: ${amount} GiB`);

    const labelXPath = "//div[text()='Amount of memory']";
    await page.waitForSelector(`xpath/${labelXPath}`);

    // --- FIX ---
    // Use page.$$ with the 'xpath/' prefix instead of page.$x
    const labelHandles = await page.$$(`xpath/${labelXPath}`);
    // --- END FIX ---

    if (labelHandles.length === 0) {
      throw new Error("Could not find the 'Amount of memory' label element.");
    }
    const labelHandle = labelHandles[0];

    const labelId = await labelHandle.evaluate(el => el.id);
    if (!labelId) {
      throw new Error("Could not find the ID of the memory label.");
    }
    console.log(`Dynamically found label ID: ${labelId}`);

    const memoryInputSelector = `input[type="number"][aria-labelledby="${labelId}"]`;

    await page.waitForSelector(memoryInputSelector, { visible: true });
    await page.click(memoryInputSelector, { clickCount: 3 });
    await page.type(memoryInputSelector, String(amount));

    console.log(`Successfully set memory to ${amount} GiB.`);

  } catch (error) {
    console.error(`Error: Failed to set the memory amount for value ${amount}.`);
    console.error(error);
    throw new Error(`Could not set memory amount: ${error.message}`);
  }
}


async function enableHighAvailabilityConfig(page) {
  // This selector is highly stable because it uses the button's accessibility label.
  const switchSelector = 'button[aria-label="Enable High Availability configuration"]';

  try {
    console.log('Checking the status of the High Availability switch...');

    // 1. Wait for the switch to be visible on the page.
    await page.waitForSelector(switchSelector, { visible: true });

    // 2. Get the current state of the switch from its 'aria-checked' attribute.
    const isEnabled = await page.$eval(switchSelector, el => el.getAttribute('aria-checked') === 'true');

    // 3. Only click the switch if it's not already enabled.
    if (!isEnabled) {
      console.log('High Availability is OFF. Enabling it now...');
      await page.click(switchSelector);

      // 4. (Optional but recommended) Wait for the state to change to confirm the click was successful.
      await page.waitForFunction(
        (selector) => document.querySelector(selector)?.getAttribute('aria-checked') === 'true',
        {},
        switchSelector
      );
      console.log('Successfully enabled High Availability configuration.');
    } else {
      console.log('High Availability is already enabled. No action needed.');
    }
  } catch (error) {
    console.error('Error during High Availability configuration.');
    console.error(error);
    throw new Error(`Could not enable High Availability: ${error.message}`);
  }
}

async function setStorage(page, amount) {
  try {
    const labelText = 'Storage (Provisioned Amount)';
    console.log(`Attempting to clear and set storage amount to: ${amount}`);

    // This robust XPath finds the input field by its proximity to the label.
    const inputXPath = `//div[contains(., "${labelText}")]/ancestor::div[contains(@class, 'MSEcuf')]//input[@type='number']`;

    // 1. Wait for the input field to be visible.
    await page.waitForSelector(`xpath/${inputXPath}`, { visible: true });
    
    // 2. Get the element handle.
    const [inputHandle] = await page.$$(`xpath/${inputXPath}`);
    if (!inputHandle) {
      throw new Error("Could not find the storage input field near the label.");
    }

    // 3. Click the input 3 times to select all text, then type the new value.
    ...
    await page.keyboard.press('Backspace');
    await sleep(500);
    await page.keyboard.press('Backspace');       // This deletes the selected text.
    await sleep(1000);
    await inputHandle.type(String(amount));      // This overwrites the selection.

    console.log(`Successfully set storage to ${amount}.`);

  } catch (error) {
    console.error(`Error in setStorage function for amount "${amount}":`);
    console.error(error);
...
async function selectCommittedUseDiscountOption(page, option) {
  try {
    // 1. Map the simplified input to the full text found on the webpage.
    let optionText;
    // Sanitize input by removing spaces and making it lowercase
    switch (String(option).toLowerCase().replace(/\s+/g, '')) {
      case '1year':
        optionText = 'Cloud SQL CUD - 1 Year';
        break;
      case '3year':
        optionText = 'Cloud SQL CUD - 3 Year';
        break;
      case 'none':
        optionText = 'None';
        break;
      default:
        throw new Error(`Invalid option: "${option}". Please use '1year', '3year', or 'None'.`);
    }

    console.log(`Attempting to select CUD option: "${optionText}"`);

    // 2. 🎯 Find the dropdown trigger by its text and role. This is very reliable.
    const dropdownTriggerXPath = `//div[contains(., 'Committed use discount options') and @role='combobox']`;
    await page.waitForSelector(`xpath/${dropdownTriggerXPath}`, { visible: true });
    
    // 3. Check if the correct option is already selected to avoid unnecessary clicks.
    const [triggerHandle] = await page.$$(`xpath/${dropdownTriggerXPath}`);
    const alreadySelected = await triggerHandle.evaluate((el, text) => el.textContent.includes(text), optionText);
    
    if (alreadySelected) {
        console.log(`"${optionText}" is already selected. No action needed.`);
        return;
    }
    ...
    // 5. Find the specific list item (<li>) by its text and click it.
    const optionXPath = `//li[@role='option'][.//span[text()="${optionText}"]]`;
    await page.waitForSelector(`xpath/${optionXPath}`, { visible: true });
    const [optionHandle] = await page.$$(`xpath/${optionXPath}`);
    
    if (!optionHandle) {
      throw new Error(`Could not find the CUD option for "${optionText}"`);
    }
    await optionHandle.click();

    // 6. (Recommended) Wait for the dropdown's visible text to update.
    await page.waitForFunction(
    ...
    console.error(`Error selecting committed use discount option "${option}":`);
    console.error(error);
    throw new Error(error.message);
  }
}
...
async function calculatePricing(sl,row, mode,isFirst, isLast) {
  ...
  let browser, page;
    ...
    try {
      ...
      await sleep(2000);
      await CloudSqlModal(page);
      await sleep(2000);
      await waitForNextSection(page);
      await sleep(500);
      await serviceType(page,row["Service Type"]);
      await sleep(500);
      ...
      if (row["Cloud SQL"] !== "Enterprise") {
        await cloudSqlEdition(page);
        await sleep(500);
      }
 

      if (row["Avg no. of hrs"] < 730) {
        //await specifyUsageTimeToggle(page)
        await sleep(500);
      }

      // works till here

      console.log(`${sl} - Setting machine type: ${row["No. of Instances"]}`);
      ...
      //await selectDBVersion(page, row["DB Version"]);
      //await selectThreadsPerCore(page, row["Threads Per Core"]); 

      await SqlInstanceType(page,row["Instance Type"]);
      await sleep(500);
      ...
      await sleep(500);
      await setStorage(page,row["Storage Amt"]);
      await sleep(500);
      //await storageType(page, row["Storage Type"]);  To do
      //await sleep(500);
      
      await sleep(500);
      await configureAdvancedSettings(page);
      await sleep(500);
      if (row["mode"]!=="ondemand"){
        await selectCommittedUseDiscountOption(page,row["mode"]);

      }
      await sleep(1000)
      if (row["High Availability"] === "HA") {
        await enableHighAvailabilityConfig(page);
      }
      await sleep(10000);
      machineInfo=await scrapeMachineTypeData(page); //works
      url=await scrapeUrl(page);  //works
      price=await scrapeEstimatedPrice(page); //works

      return {
        price,
        url,
        machineType: machineInfo.machineType || 'Unknown',
